...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const breakTimeH   = params.get("breakTimeH")  || "0";
  const breakTimeM   = params.get("breakTimeM")  || "0";
  ...
  document.getElementById("confirm-status").textContent =
    ATTENDANCE_STATUS_MAP[statusId] || statusId;

  // 4) 出退勤・休憩時刻フィールドにセット
  document.getElementById("confirm-start").textContent = startTime;
  document.getElementById("confirm-end").textContent   = endTime;
  document.getElementById("confirm-break").textContent = `${breakTimeH}時間${breakTimeM}分`;
  ...
  document.getElementById("submit-form")
    ...
    const payload = {
      ...
      startTime:   startTime,
      endTime:     endTime,
      breakTimeH:  breakTimeH,
      breakTimeM:  breakTimeM
    };
